...
if (0) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  errs(iter)  = mse(obj,Xtr,YtrK);                              % surrogate (mse on output)
  %surr(iter)  = -loglikelihood(obj,Xtr,YtrK);                   % surrogate (neg likelihood)

  % Make plots, if desired
  if (plotFlag),
    %fig(1); semilogx(1:iter, surr(1:iter),'b-',1:iter,errs(1:iter),'g-');
    fig(1); semilogx(1:iter,errs(1:iter),'g-');
